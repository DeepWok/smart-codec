...
from .hamming import HammingDataset
...
def get_dataset(name, mode, my_path=None, transform=transforms.ToTensor()):
    name = name.lower()
    if name in ['hamming', 'Hamming']:
        return HammingDataset(transform=transform, mode=mode, path=my_path)
